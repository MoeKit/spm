...
module.exports = function(argv, callback) {
  ...
  if (argv.upload) return scp(argv, callback);
};
...
function scp(argv, callback) {
  var host = spmrc.get('scp.host');
  if(!host){
    return log.error('PLEASE USE moekit config set scp.host yourpath TO SET SERVER PATH');
  }else{
  ...
  client.scp(dir, host + config.name + '/', function(err) {
    if (!err) {
